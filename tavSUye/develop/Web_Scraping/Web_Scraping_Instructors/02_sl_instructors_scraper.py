...
base_url_en = "https://sl.sabanciuniv.edu/en/faculty-members"
base_url_tr = "https://sl.sabanciuniv.edu/tr/faculty-members"
domain = "https://sl.sabanciuniv.edu"
...
def extract_instructor_links():
    ...
    for card in cards:
        ...
        image_url = None
        figure_tag = card.select_one("figure.card-picture")
        if figure_tag and "background-image" in figure_tag.get("style", ""):
            image_url = "https:" + figure_tag["style"].split("url('")[1].split("')")[0]
...
def enrich_with_about(instructors):
    ...
    for i, inst in enumerate(instructors, start=1):
        ...
        inst["about_tr"] = extract_about(inst["link_tr"], "Areas of Interest")  # SL TR sayfası da İngilizce başlık kullanıyor
    return instructors

def main():
    print("🚀 Starting instructor scraping process for SL...")
    ...
    print("💾 [3/3] Saving results to sl_instructors.json...")
    with open("sl_instructors.json", "w", encoding="utf-8") as f:
        json.dump(instructors, f, indent=2, ensure_ascii=False)

    print(f"\n✅ SUCCESS: Saved {len(instructors)} instructors to sl_instructors.json.")
